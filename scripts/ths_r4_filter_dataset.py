...
ANNES_12_SWRG_LOCS = ['Auckland', 'Blenheim', 'Christchurch', 'Dunedin', 'Gisborne', 'Greymouth', 'Masterton', 'Napier', 'Nelson', 'Queenstown', 'Tauranga', 'Wellington']

@click.command()
@click.argument('source')
@click.argument('target')
@click.option('-L', '--locations', help="one or more location identifiers (comma-separated). Use any valid nzshm_location identifier")
...
def main(
    ...
    if not locations:
        locations = [loc['id'] for loc in location.LOCATIONS if (loc['name'] in ANNES_12_SWRG_LOCS and loc['id'][:3] == "srg")]
    ...
    for partition_code, partition_bin in  partition_bins.items():
        for loc in partition_bin.locations:
            flt0 = (pc.field('nloc_0') == pc.scalar(partition_code)) &\
